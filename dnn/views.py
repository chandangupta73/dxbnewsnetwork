...
def home(request):
    ...
    articales=NewsPost.objects.filter(schedule_date__lt=current_datetime,articles=1,status='active').order_by('-id') [:8]
    ...
    for cat in Category:
        ...
        category_posts = NewsPost.objects.filter(
            Q(post_cat__in=subcategories),
            is_active=1,
            status='active',
...
# -------------end-ad-manage-meny--------------    
    ...
    data={
            ...
            'recipe':recipe,
            'tt':truet,
            'grouped_postsdata': grouped_postsdata,
        }
...
def newsdetails(request,slug):
    ...
    seo='ndetail'
    current_datetime = datetime.now()
    blogdetails=NewsPost.objects.get(slug=slug, is_active=1,status='active')
    blogdata=NewsPost.objects.filter(schedule_date__lt=current_datetime,is_active=1,status='active').order_by('-id') [:10]
